        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for c in range(4):
                    ...
                    if x is not None and len(x) and c == 1:
                        w = x[:, 2] - x[:, 0]
                        h = x[:, 3] - x[:, 1]
                        ratio = np.maximum(w / h, h / w)
                        mask = ratio <= 3
                        x = x[mask]
                    ...
                    if x is not None and len(x):
                        ...
                        for t in online_targets:
                            tlwh = t.tlwh
                            tid = t.track_id
                            ratio = max(tlwh[2] / tlwh[3], tlwh[3] / tlwh[2])
                            ...
                            if tid not in static_dict.keys():
                                ...
                                static_dict[tid][0] += 1
                                static_dict[tid][1] = (static_dict[tid][1] * static_dict[tid][0] + area) / (
                                        static_dict[tid][0] + 1)
                                if static_dict[tid][0] > linear_projection(static_dict[tid][1], area_boundary,
