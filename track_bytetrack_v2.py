...
def plot_tracking(image, tlwhs, obj_ids, frame_id=0, fps=0., ids2=None, online_cxcy=None):
    # 画图的
    ...
    if online_cxcy is not None:
        for i, cxcy in enumerate(online_cxcy):
            x, y = cxcy
            cv2.circle(im, (int(x), int(y)), 1, (0, 0, 255), 4)
    return im
...
def run(
    ...
    roi_points = {
        2: [(500, 200), (800, 1080), (200, 1080)],
        4: [(1120, 100), (1420, 300), (1720, 200), (1720, 1080), (900, 900)]
    }
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in tqdm(enumerate(dataset)):
        ...
        for i, det in enumerate(pred):  # detections per image
                            ...
                            for roi_index, roi_ in roi.items():
                                ...
                                if tid not in static_dict[roi_index][c].keys():
                                    ...
                                    if static_dict[roi_index][c][tid][0] > linear_projection(
                                            ...
                                            count_thres) and tid not in all_count[roi_index][-1] and \
                                            static_dict[roi_index][c][tid][-1]:
            ...
            online_im = cv2.putText(online_im, f'people: {get_postfix(0)}', (50, 100),
                                    cv2.FONT_HERSHEY_SIMPLEX,
                                    1, (255, 0, 0), 2, cv2.LINE_AA)
            online_im = cv2.putText(online_im, f'car: {get_postfix(1)}', (50, 150),
                                    cv2.FONT_HERSHEY_SIMPLEX, 1,
                                    (255, 0, 0), 2, cv2.LINE_AA)
            online_im = cv2.putText(online_im, f'bus: {get_postfix(2)}', (50, 200),
                                    cv2.FONT_HERSHEY_SIMPLEX, 1,
                                    (255, 0, 0), 2, cv2.LINE_AA)
            online_im = cv2.putText(online_im, f'truck: {get_postfix(3)}', (50, 250),
                                    cv2.FONT_HERSHEY_SIMPLEX,
                                    1, (255, 0, 0), 2, cv2.LINE_AA)
            online_im = cv2.putText(online_im, f'all: {get_postfix(4)}', (50, 300),
                                    cv2.FONT_HERSHEY_SIMPLEX,
                                    1, (255, 0, 0), 2, cv2.LINE_AA)
