...
from tracker.byte_tracker import BYTETrackerWithFeat
...
def run(
    ...
    trackers = [BYTETrackerWithFeat(opt, frame_rate=8) for _ in range(4)]
    timer = Timer()
    # Load model
    ...
    for frame_idx, (path, im, im0s, vid_cap, s) in tqdm(enumerate(dataset)):
        ...
        for i, det in enumerate(pred):  # detections per image
                ...
                for c in range(4):
                    ...
                    if x is not None and len(x):
                        ...
                        for t in online_targets:
                            ...
                            if tid not in static_dict.keys():
                                static_dict[tid] = [0, area]
                            else:
                                static_dict[tid][0] += 1
                                static_dict[tid][1] = (static_dict[tid][1] * static_dict[tid][0] + area) / (static_dict[tid][0] + 1)
                                if static_dict[tid][0] > linear_projection(static_dict[tid][1], area_boundary, count_thres) and tid not in all_count[-1]:
            ...
            online_im = cv2.putText(online_im, 'people: ' + str(len(all_count[0])), (50, 100), cv2.FONT_HERSHEY_SIMPLEX,
                                    1, (255, 0, 0), 2, cv2.LINE_AA)
            online_im = cv2.putText(online_im, 'car: ' + str(len(all_count[1])), (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                    (255, 0, 0), 2, cv2.LINE_AA)
            online_im = cv2.putText(online_im, 'bus: ' + str(len(all_count[2])), (50, 200), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                    (255, 0, 0), 2, cv2.LINE_AA)
            online_im = cv2.putText(online_im, 'truck: ' + str(len(all_count[3])), (50, 250), cv2.FONT_HERSHEY_SIMPLEX,
                                    1, (255, 0, 0), 2, cv2.LINE_AA)
    ...
    with open(txt_path, 'a+') as f:
        s = ' '.join(map(str, [len(i) for i in all_count]))
        s = source + ' ' + s
        print(s)
        f.write(s + '\n')
    # Print results
...
def parse_opt():
    ...
    parser.add_argument('--reid-weights', default=WEIGHTS / 'osnet_x0_25_msmt17.pt')
