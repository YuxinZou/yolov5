...
from utils.torch_utils import (fuse_conv_and_bn, initialize_weights, model_info,
                               profile, scale_img, select_device,
                               time_sync)
...
class DecoupledDetect(Detect):
    ...
    def __init__(self, nc=80, anchors=(), ch=(),
        ...
        self.m = nn.ModuleList(
            Decoupled(x, nc, 1, anchors) for x in ch)  # output conv
...
class Model(nn.Module):
    ...
    def __init__(self, cfg='yolov5s.yaml', ch=3, nc=None,
        ...
        if isinstance(m, (Detect, DetectIouAware, DecoupledDetect)):
            ...
            m.stride = torch.tensor([s / x.shape[-2] for x in self.forward(
                torch.zeros(1, ch, s, s))])  # forward
            check_anchor_order(m)  # must be in pixel-space (not grid-space)
            # 处理anchor到当前featmap上的大小
    ...
    def forward(self, x, augment=False, profile=False, visualize=False):
        ...
        return self._forward_once(x, profile,
                                  visualize)  # single-scale inference, train
    ...
    def _descale_pred(self, p, flips, scale, img_size):
        ...
        if self.inplace:
            ...
            x, y, wh = p[..., 0:1] / scale, p[..., 1:2] / scale, p[...,
                                                                 2:4] / scale  # de-scale
            if flips == 2:
    ...
    def _clip_augmented(self, y):
        ...
        y[0] = y[0][:, :-i]  # large
        i = (y[-1].shape[1] // g) * sum(
            4 ** (nl - 1 - x) for x in range(e))  # indices
        y[-1] = y[-1][:, i:]  # small
        return y
    ...
    def _initialize_biases(self,
        ...
        if isinstance(m, DecoupledDetect):
            ...
            for mi, s in zip(m.m, m.stride):  # from
                ...
                b[:, 5:] += math.log(
                    0.6 / (m.nc - 0.999999)) if cf is None else torch.log(
                    cf / cf.sum())  # cls
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    no = na * (
            nc + 5)  # number of outputs = anchors(3) * (classes + 5) 3 * (80+5)= 255

    # c1=c_in, c2=c_out
    layers, save, c2 = [], [], ch[-1]  # layers, savelist, ch out
    for i, (f, n, m, args) in enumerate(
            d['backbone'] + d['head']):  # from, number, module, args
