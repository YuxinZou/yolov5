        ...
        for i, det in enumerate(pred):  # detections per image
                    ...
                    if x is not None and len(x):
                        ...
                        for t in online_targets:
                            ...
                            for roi_index, roi_ in roi.items():
                                ...
                                if id_in and tid not in id_mapping.keys():
                                    id_count += 1
                                ...
                                if not id_in:
                                    online_ids[-1] = -1
                timer.toc()
                online_im = plot_tracking(im0, online_tlwhs, online_ids, id_mapping, online_labels, font=font, txt_color=txt_color)
            else:
                timer.toc()
                online_im = im0
            ...
            def get_postfix(c):
                ...
                for roi_index, _ in roi.items():
                    s += f'{str(len(all_count[roi_index][c]))} '
                return s
            ...
            # online_im = (0.1 * mask + 0.9 * online_im).astype(np.uint8)
